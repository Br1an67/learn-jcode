...
JCode 可以先看成一个本地 coding-agent runtime。模型负责判断下一步，JCode 负责把工具、上下文、权限、状态和 UI 接起来。

这份教程不要求你先跳到源码里找文件。每一课会摘出关键代码，解释它在整个 runtime 里的位置。
...
| 课次 | 讲什么 | 在总图里的位置 |
| --- | --- | --- |
| [s01](./s01-harness-mindset.md) | 先把模型和 harness 的边界讲清楚 | 整张图的读法 |
| [s02](./s02-startup-server.md) | `jcode` 怎么启动 server/client | `TUI client`、`resident server` |
| [s03](./s03-agent-loop.md) | 一轮模型输出、工具调用、工具结果怎么循环 | `agent loop` |
| [s04](./s04-tool-system.md) | 工具 schema 和执行入口怎么统一 | `tool registry` |
| [s05](./s05-provider-session.md) | provider/auth/session 为什么变复杂 | `provider layer`、`session` |
| [s06](./s06-tui-observability.md) | TUI 怎样把 runtime 状态变成用户判断 | `protocol events`、`TUI client` |
| [s07](./s07-memory.md) | memory 为什么是非阻塞 sidecar | `memory sidecar` |
| [s08](./s08-swarm.md) | swarm 为什么是 server-level coordination | `swarm state / channels` |
| [s09](./s09-ambient-selfdev.md) | ambient 和 self-dev 的后台边界 | `scheduler`、`selfdev tool`、`reload` |
| [s10](./s10-comparison.md) | JCode 和其他 coding-agent runtime 的边界 | 回到整张图做对照 |
