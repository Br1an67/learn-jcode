...
下面代码只看三件事：ambient 不是一个单独工具文件；ready item 由持久化队列拿出来；后台 agent 必须通过 `end_ambient_cycle` 交代本轮结果。
...
两边都绕不开恢复：ambient 用 queue 恢复下一次唤醒，self-dev 用 manifest 和 reload context 恢复正在做的修改。
